...
used_class_nums = set((i for i in range(16,32)))
sup_ratio = 0.05
def divide_label_unlabel(
    ...
    # if 0 not in used_class_nums:
    #     class_set = set((i for i in range(0,min(used_class_nums)+1)))
    #     last_dataset_dicts = copy.deepcopy(dataset_dicts)
    #     for data in last_dataset_dicts:
    #         annotations = []
    #         for anno in data['annotations']:
    #             if anno['category_id'] in class_set:
    #                 annotations.append(anno)
    #         data['annotations'] = annotations
    #     last_phase_dataset = [x for x in last_dataset_dicts if len(x['annotations']) > 0]
    #     replay_dicts = last_phase_dataset[:int(len(last_phase_dataset) * sup_ratio)]
        
    # ------------------Replay-----------------------------------
    for data in dataset_dicts:
        annotations = []
        for anno in data['annotations']:
            if anno['category_id'] in used_class_nums:
                annotations.append(anno)
        data['annotations'] = annotations
    dataset_dicts = [x for x in dataset_dicts if len(x['annotations']) > 0]
    label_dicts = dataset_dicts[:int(len(dataset_dicts) * sup_ratio)]
    label_dicts.extend(replay_dicts)
    unlabel_dicts = dataset_dicts[int(len(dataset_dicts) * sup_ratio):len(dataset_dicts)]
    print(f"sup_ratio = {sup_ratio}, used_class_nums = {used_class_nums}")
    print(f"label data size {len(label_dicts)}, unlabel data size {len(unlabel_dicts)}")
